...
class GPUManager:
    ...
    def get_free_gpu(self):
        ...
        while True:
            ...
            if gpu_ind is not None:
                return gpu_ind
            if self.verbose:
                print('pid {} sleeping'.format(os.getpid()))
    ...
    def _get_free_gpu(self):
        ...
        if available_gpu_inds:
            ...
            if self.verbose:
                print('pid {} picking gpu {}'.format(os.getpid(), gpu_ind))
            if self.lock_manager.lock('gpu_{}'.format(gpu_ind), GPU_LOCK_TIMEOUT):
                return int(gpu_ind)
            if self.verbose:
                print('pid {} couldnt get lock'.format(os.getpid()))
        return None